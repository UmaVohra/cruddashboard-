...
function Signin() {
  ...
  const handleSignin = async(e) => {
    ...
    if (!Password) {
      newErrors.Password = "It cant be empty";
    }
    setErrors(newErrors);
  ...
  return (
    <div className=" flex items-center justify-center h-screen bg-gray-900">
      ...
      <h1 className="text-3xl block font-bold text-center text-white">SIGNIN</h1>
      <form onSubmit={handleSignin}>
        ...
        <button className="bg-blue-700 mt-4 text-xl
         text-white hover:bg-transparent hover:text-blue-50 font:semi-bold 
         rounded w-full cursor-pointer  ">Sign in! </button>
        <br />

        <p className="text-center font-bold mt-3 text-blue-600">Don't have an account <Link to="/signup">signup</Link></p>
      </form></div>
    </div>
  );
}
